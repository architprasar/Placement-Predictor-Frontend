...
const Cocubes = ({ setModel }) => {
    ...
    const [computerFundamental, setComputerFundamental] = useState('');
    ...
    const [writtenEnglish, setWrittenEnglish] = useState('');
    ...
    const callApi = async (fromData) => {
        axios.post(' http://127.0.0.1:8000/api/cocubes/', fromData)
            .then((response) => {
                ...
                document.getElementById("trustscore").innerText = "Trust Score: " + response.data.trust
            })
            .catch((error) => {
    ...
    const handleSubmit = (event) => {
        ...
        const formData = {
            ...
            "Analytical": String(analytical),
            "Quantitative": String(quantitative),
            "English": String(english),
            "Aptitude": String(aptitude),
        ...
        <Box style={{ display: 'flex', height: "100%", justifyContent: "center", flexShrink: 0, }}>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <Box style={{ display: 'flex', flexDirection: 'column', width: "500px", marginTop: "20px", marginBottom: "50px" }}>
                    ...
                    />
                    <FormControl>
                        <InputLabel>Coding</InputLabel>
                        <Select
                            value={coding}
                            onChange={(event) => setCoding(event.target.value)}
                        >
                            <MenuItem value="Good to go">Good to go</MenuItem>
                            <MenuItem value="Need training">Need training</MenuItem>

                        </Select>
                    </FormControl>
                    <FormControl>
                        <InputLabel>Computer Fundamental</InputLabel>
                        <Select
                            value={computerFundamental}
                            onChange={(event) => setComputerFundamental(event.target.value)}
                        >
                            <MenuItem value="Good to go">Good to go</MenuItem>
                            <MenuItem value="Need training">Need training</MenuItem>
                        </Select>
                    ...
                    />
                    <FormControl>
                        <InputLabel>Written English</InputLabel>
                        <Select
                            value={writtenEnglish}
                            onChange={(event) => setWrittenEnglish(event.target.value)}
                        >
                            <MenuItem value="Good to go">Good to go</MenuItem>
                            <MenuItem value="Need training">Need training</MenuItem>
                        </Select>
